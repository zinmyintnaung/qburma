...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form action="{{ route('explanationdetail.store') }}" method="post">
                {{ csrf_field() }}
                <div class="form-group">
                    ...
                    <select name="sura_text_id" id="suratext" class="form-control">
                        @foreach($suratexts as $suratext)
                            <option value="{{ $suratext->id }}">{{ $suratext->id }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="description">Explanation Detail</label>
                    <textarea name="explanation_detail" id="explanation_detail" cols="5" rows="5" class="form-control"></textarea>
                </div>
